...
The artifact boundary uses the official Node 24 releases at immutable commits:
...
Both upstream `action.yml` files must declare `runs.using: node24`. Insecure
Node fallback is prohibited. The ordinary, non-publishing test workflow must
exercise the same named archive shape as the release boundary: upload a `dist/`
tree with a one-day retention, download it into a fresh directory, and compare
the SHA-256 manifest byte-for-byte. It must also prove that an absent upload path
fails when `if-no-files-found` is `error`. The qualification job has read-only
repository permission and no package-index identity or publishing step.
